...
import { Plus, MessageCircle, WifiOff, ShoppingCart } from 'lucide-react';
...
const ProductCard: React.FC<{ product: any }> = ({ product }) => {
  ...
  const handleWhatsApp = (e: React.MouseEvent) => {
    ...
    const text = encodeURIComponent(`${t('checkout.whatsapp_message_start')} ${product.name} (Réf: ${product.id}).`);
  ...
  return (
    <div 
      ...
      className={`rounded-2xl overflow-hidden shadow-sm border flex flex-col cursor-pointer active:scale-[0.98] transition-transform ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100 shadow-sm'}`}
    >
      <div className="relative">
        <OptimizedImage src={product.image} alt={product.name} className="w-full aspect-square" />
        <div className="absolute top-2 left-2">
          {product.inStock ? (
            <span className="bg-green-100 text-green-800 text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-wide shadow-sm">{t('catalog.available')}</span>
          ) : (
            <span className="bg-red-100 text-red-800 text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-wide shadow-sm">{t('catalog.out_of_stock')}</span>
          )}
        </div>
      </div>
      <div className="p-3 flex flex-col flex-grow justify-between gap-3">
        ...
        <div className="flex flex-col gap-2">
          ...
          >
            <MessageCircle size={18} /> {t('catalog.whatsapp')}
          </button>
        </div>
      </div>
    </div>
...
export const Catalog = () => {
  const { networkStatus, theme, t, navigate } = useAppContext();
  const isDark = theme === 'dark';
  const [activeCategory, setActiveCategory] = useState('Tous');

  const CATEGORIES = [
    { id: 'Tous', label: t('catalog.all') },
    { id: 'Gâteaux', label: t('catalog.cakes') },
    { id: 'Pâtisseries', label: t('catalog.pastries') },
    { id: 'Kits', label: t('catalog.kits') }
  ];
  ...
  return (
    <section className={`px-4 py-6 max-w-md mx-auto md:max-w-4xl pb-24 transition-colors duration-300 ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className={`text-2xl font-extrabold ${isDark ? 'text-white' : 'text-gray-900'}`}>{t('catalog.title')}</h1>
        <button 
          onClick={() => navigate('public-tracking')}
          className="flex items-center space-x-2 px-4 py-2 rounded-xl bg-amber-500/10 text-amber-600 text-xs font-black uppercase tracking-wider active:scale-95 transition-transform"
        >
      ...
      <div className="flex overflow-x-auto hide-scrollbar gap-2 mb-6 pb-2">
        {CATEGORIES.map(cat => (
          <button
            ...
            className={`whitespace-nowrap px-4 py-2 rounded-full text-sm font-bold min-h-[48px] transition-colors ${activeCategory === cat.id ? 'bg-amber-500 text-white shadow-sm' : (isDark ? 'bg-gray-800 text-gray-400 active:bg-gray-700' : 'bg-gray-100 text-gray-600 active:bg-gray-200')}`}
          >
            {cat.label}
          </button>
        ))}
